...
# Get list of skills from _system and _integrations directories
SKILLS=$( (ls -1 "${SKILLS_DIR}/_system/" && ls -1 "${SKILLS_DIR}/_integrations/") 2>/dev/null | sort)
...
test_skill() {
    local skill=$1
    # Find skill in _system or _integrations
    if [ -d "${SKILLS_DIR}/_system/${skill}" ]; then
        local skill_path="${SKILLS_DIR}/_system/${skill}"
    elif [ -d "${SKILLS_DIR}/_integrations/${skill}" ]; then
        local skill_path="${SKILLS_DIR}/_integrations/${skill}"
    else
